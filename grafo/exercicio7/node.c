...
struct node {
  ...
  float distancia;   /*CHAMO A MATRICULA DO ALUNO DE ID*/
  struct node *next; /*APONTA PARA O PROXIMO ELEMENTO*/
};

/*CRIO UM nó DA LISTA COM UM VALOR JA DEFINIDO*/
struct node *node_creat(int id, float distancia) {
...
float get_distancia(struct node *n) { return n->distancia; }

/*ADIONO UM NOME*/
void set_distancia(struct node *n, float distancia) {
